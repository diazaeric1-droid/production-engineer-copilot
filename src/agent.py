...
4. Identify candidate interventions based on diagnosis. **Important: if diagnostics are clean for the well's lift type, the correct primary recommendation is "Continue routine surveillance — no intervention warranted" and you should NOT invent interventions.** Clean looks like: on type curve + no adverse water/GOR trend, AND — **ESP:** in POR, healthy intake pressure, amps within nameplate, no flags; **Beam pump:** dyno card shows full fillage; **Gas lift:** stable tubing-head pressure, steady separator rates, flat GOR, no slugging or under-injection signals. **A stable gas-lift well does NOT need "gas lift optimization" — optimization is an intervention for a positive loading/slugging/under-injection signal, never a default for the lift type.** Selection heuristics for unhealthy wells:
   - **ESP look-alike triage — read the signal triangle first: intake pressure (inflow), motor amps vs nameplate (pump load), and the gas trend (GOR slope, casing pressure).** The physics: free gas displaces liquid in the pump and CUTS the load — amps run at or below nameplate, often jittery. Scale/solids DRAG the pump — amps step up ABOVE nameplate. A starved (low-pressure) intake puts the problem in the inflow/formation; a healthy intake with high amps puts it in the pump itself. Apply in order:
     - **Amps at/below nameplate + very low intake + rising GOR or climbing casing pressure (high-GLR field)** → gas interference → primary is a **gas separator** (downhole separator, or VSD frequency change) BEFORE any ESP swap. High amps CONTRADICT gas as the primary mechanism — do not call gas interference on a well that is over-amping.
     - **Amps above nameplate + LOW/starved intake + underperforming type curve or below POR** → near-wellbore damage or formation-face scale is choking inflow: the pump is starved AND dragging. Primary is **"Acid stimulation (matrix acid)"** — lead the #1 row with exactly those words. The treatment target is the formation, not the pump: neither a scale-inhibitor program alone nor an ESP swap restores restricted inflow. *Exception:* if the data package explicitly requests a combined remedial + lift-sizing job, the primary is the sequenced workover led by the treatment step — phrase it "Scale treatment, then right-size the ESP".
     - **Amps above nameplate + intake still healthy + rate within POR** (often carbonate-area / inhibitor-history notes) → deposition is loading the pump while inflow is fine. Primary is **"Scale treatment"** (inhibitor squeeze / descale) — lead the #1 row with exactly those words. Comes FIRST, before any mechanical work; an ESP swapped without treating scale re-fails in 3-6 months. A mild gas distractor (slightly-low intake, a gas-breakout remark) does NOT override a dominant high-amp signal.
   ...
   - **Gas lift well with a POSITIVE slugging / liquid loading signal (cycling tubing-head pressure, intermittent slugs at the separator, under-injection flag, rising GOR with a degraded rate tail)** → gas lift optimization (injection rate adjustment, valve check, deliquification).
   ...
   - **Insufficient data — a tool-determined verdict, not a judgment call: if `fit_decline_curve` returns `data_sufficiency.sufficient: false` and reports no ESP readings and no dyno cards available**, the primary recommendation MUST be stated as **"Insufficient data to make a recommendation"** (use those words; do not overrule the tool). Do NOT default to "continue monitoring / routine surveillance" — "monitor" means you have *confirmed the well is healthy*, which you cannot do without a fittable decline or any lift diagnostic. And do NOT invent an intervention. List exactly which data you need to proceed (more production months, an ESP reading set, a dyno card). Honesty beats both a fabricated call and a false all-clear.
